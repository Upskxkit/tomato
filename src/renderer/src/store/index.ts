...
export const useTimersStore = create<TimersState>((set) => ({
    ...
    updateTimer: (timer: Timer) => set((state) => ({ timers: state.timers.map(item => item.id === timer.id ? timer : item) })),
    addTimer: (timer: Timer) => set((state) => ({ timers: [...state.timers, timer] })),
    deleteTimer: (timer: Timer) => set((state) => ({ timers: state.timers.filter(item => item.id !== timer.id) }))
}));
...
export const useTimerModalStore = create<TimerModalState>((set) => ({
    ...
    open: (timer?: Timer) => set((state) => ({ ...state, visible: true, timer: timer || { time_in_sec: 0, title: '' }, mode: timer ? 'edit' : 'create' })),
}));
