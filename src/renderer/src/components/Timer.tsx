...
export type TimerProps = { timer: TimerType; onChange?: (props: TimerType) => void, onDelete?: (props: TimerType) => void, onTimeOut?: () => void };
...
export const Timer = (props: TimerProps) => {
    ...
    return (
        <>
            <Card hoverable className="timer" title={props.timer.title} extra={<a href="#">More</a>}
                actions={[
                    <Tooltip title="search">
                        <Button type="primary" shape="circle" icon={play ? <PauseOutlined /> : <CaretRightOutlined />} onClick={() => {
                    ...
                    <Tooltip title="reset">
                        <Button disabled={!isRunning} type="primary" shape="circle" icon={<RollbackOutlined />} onClick={() => {
                            setIsRunning(false);
                            setPlay(false);
                            setTimeLeft(props.timer.time_in_sec);
                        }} />
            ...
            >
                <Flex align="center" justify="center" >
                    <Progress type="circle" percent={timerInfo.percent} status="normal" format={() => timerInfo.time} size={200} />
                </Flex>
            </Card>
            <TimerPopup {...popup} isRunning={isRunning} onClick={(event) => {
